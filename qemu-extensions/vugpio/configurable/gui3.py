...
def build_board_from_config() -> BoardSpec:
    toggles: List[InputToggle] = []
    buttons: List[InputButton] = []
    indicators: List[OutputIndicator] = []

    for line in BOARD_LINES:
        if line.role == "switch":
            toggles.append(InputToggle(line.gpio, line.name))
        elif line.role == "button":
            buttons.append(
                InputButton(
                    line.gpio,
                    line.name,
                    released_value=1 if line.initial_value else 0,
                    pressed_value=0 if line.initial_value else 1,
                )
            )
        elif line.role == "led":
            indicators.append(OutputIndicator(line.gpio, line.name))

    return BoardSpec(toggles=toggles, buttons=buttons, indicators=indicators)


DEMO_BOARD = build_board_from_config()
